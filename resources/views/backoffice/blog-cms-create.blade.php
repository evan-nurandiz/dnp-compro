...
<div class="relative overflow-x-auto h-screen p-4">
    <div class="bg-gray-50 rounded-b-lg rounded-r-lg">
        <form class="p-4" method="post" action="{{route('admin-cms-blog-store')}}" enctype="multipart/form-data">
            @csrf
            {{ method_field('POST') }}
            <div class="mb-4">
                ...
                " 
                placeholder="Blog Title" required name="blog_title" value="">
                @error('blog_title')
            ...
            <div class="mb-4">
                ...
                <textarea id="editor" name="blog_body" id="blog_body"></textarea>
                @error('blog_body')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
