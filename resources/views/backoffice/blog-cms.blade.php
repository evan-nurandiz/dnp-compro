        ...
        <table class="w-full text-sm text-left text-gray-500 ">
            ...
            <tr class="bg-gray-50 border-b last:border-none">
                <th scope="row" class="px-6 py-4 font-medium text-gray-900  whitespace-nowrap  h-[150px]">
                    <img src="{{asset('storage/images/'.$blog->blog_thumbnail)}}" alt="" class="h-[150px]">
                </th>
                <td class="px-6 py-4">
                    {{$blog->blog_title}}
                </td>
                <td class="px-6 py-4">
                    {{ \Carbon\Carbon::parse($blog->updated_at)->format('d M y m:s')}}
                </td>
                <td class="px-6 py-4 text-center flex justify-center items-center mt-auto h-[182.5px]">
                    <a href="{{route('admin-cms-blog-edit',[
                        'id' => $blog->blog_id    
                    ...
                    <form action="{{route('admin-cms-blog-delete',[
                            'id' => $blog->blog_id    
                        ])}}" method="post">
                        @csrf
                        {{ method_field('DELETE') }}
                        <button href="{{route('admin-cms-blog-delete',[
                            'id' => $blog->blog_id    
